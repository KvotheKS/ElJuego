...
var jetpackFullAudio = preload("res://Assets/Sounds/Feedback1.wav")
var jetpackAudio = preload("res://Assets/Sounds/Jetpack.wav")
...
func calculate_jetpack_overheat(overheat: float,
	...
	if (outHeat == 0 and overheat != 0):
		if !$JetpackFullAudio.is_playing():
			$JetpackFullAudio.stream = jetpackFullAudio
			$JetpackFullAudio.play()
